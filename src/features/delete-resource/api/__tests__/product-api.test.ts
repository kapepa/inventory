...
import { requestDeleteProduct } from '../product-api'
...
describe('Delete Product API', () => {
  ...
  it('successfully deletes product', async () => {
    vi.mocked(axiosClient.delete).mockResolvedValue({ data: mockProduct })

    const result = await requestDeleteProduct({ id: 'product-123', signal: mockSignal })
  ...
  it('throws ForbiddenError on 403', async () => {
    ...
    await expect(requestDeleteProduct({ id: 'product-123', signal: mockSignal }))
  ...
  it('calls correct endpoint', async () => {
    ...
    await requestDeleteProduct({ id: 'product-123', signal: mockSignal })

    expect(axiosClient.delete).toHaveBeenCalledWith('/products/product-123', { signal: mockSignal })
  })
  ...
  it('passes id in URL', async () => {
    ...
    await requestDeleteProduct({ id: 'test-id-456', signal: mockSignal })
  ...
  it('passes abort signal to axios', async () => {
    ...
    await requestDeleteProduct({ id: 'product-123', signal: customSignal })
